# Guion de Presentación: Proyecto DSL (E-commerce Urban Oversize)
...
*   **Explicación del Metamodelo en nuestro Proyecto:**
    *   **Sintaxis Abstracta:** Nuestras reglas de negocio en Node.js (separar ropa de hombre y mujer, calcular precios).
...
*   *Acción:* Abre la página principal del E-commerce (Urban Oversize).
*   **Punto clave:** "Profesor, este diseño limpio estilo Zara que está viendo no contiene ni un solo bloque `forEach` en la vista. Todo fue generado en el servidor por el motor del DSL Handlebars".
...
for(let i = 0; i < menProducts.length; i++) {
    ...
    html += '<h3>' + menProducts[i].name + '</h3>';
    if(menProducts[i].isNewArrival) {
...
*   **Componentización (Partials):** Usar `{{> productCard}}` nos permitió crear la tarjeta de la prenda de ropa una sola vez y reciclarla en el catálogo de hombres y mujeres.
